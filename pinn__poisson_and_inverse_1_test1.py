...
def trudx(x, y):
    trudx=np.exp(x*x+y*y)*2*x
    return trudx

def trudy(x, y):
    trudy=np.exp(x*x+y*y)*2*y
    return trudy
k0=4
def f(x,y,mu):
  tru = mu*(x*x+y*y+1)*torch.exp(x*x+y*y)
  return tru
...
# training .....
epochs = 80000
mu = torch.nn.Parameter(torch.zeros(1, requires_grad=True)) #定義了一個可學習的參數mu(標量),用於估計微分方程中的未知常數
# optimiser = torch.optim.Adam(list(pinn.parameters())+[mu],lr=1e-3) #初始化優化器(使用Adam優化算法)
optimiser1 = torch.optim.Adam(pinn.parameters(),lr=1e-4) #初始化優化器(使用Adam優化算法)
optimiser2 = torch.optim.Adam([mu],lr=1e-1) #初始化優化器(使用Adam優化算法)

mus = []
loss_values = np.array([])
L_values = np.array([])
l_values = np.array([])
# lambda1, lambda2 = 1e-0, 3e-1 #控制不同loss的權重
start = time.time()
for epoch in range(epochs):
    ...
    loss = L + l
    loss.backward()   #計算加權總損失loss,對其進行反向傳播來獲得所有參數的梯度
    optimiser1.step()   #使用優化器的step()方法更新模型參數
    optimiser2.step()
...
plt.hlines(k0, 0, len(mus), label="True value", color="tab:blue")
plt.legend()
...
ax3 = fig.add_subplot(133, projection='3d')
surf3 = ax3.plot_surface(X0, Y0, TT3, cmap='rainbow')
ax3.set_xlabel("x", fontsize=16)
ax3.set_ylabel("y", fontsize=16)
ax3.set_zlabel("f_nn-f", fontsize=16)
ax3.set_title("different", fontsize=16)
...
for i in fig_nums:
    plt.figure(i)
    plt.savefig(f'PINN poisson and inverse_1 test1 運行結果/figure_{i}.png')