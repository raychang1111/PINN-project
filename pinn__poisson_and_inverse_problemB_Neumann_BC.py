...
class FCN(nn.Module):
  ...
  def __init__(self, N_INPUT, N_OUTPUT, N_HIDDEN, N_LAYERS):
    ...
    self.fch = nn.Sequential(*[nn.Sequential(*[nn.Linear(N_HIDDEN, N_HIDDEN),activation()]) for _ in range(N_LAYERS-1)]) # 重複 N_LAYERS - 1 次，以創建多個隱藏層
...
# training .....
epochs = 80000
mu = torch.nn.Parameter(torch.zeros(2, requires_grad=True)) #定義了兩個可學習的參數mu(標量),用於估計微分方程中的未知常數
...
for i in fig_nums:
    plt.figure(i)
    plt.savefig(f'PINN poisson and inverse_2 test2 運行結果/figure_{i}.png')
